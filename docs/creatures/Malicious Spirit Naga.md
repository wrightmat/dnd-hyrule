---
statblock: inline
---
 #monster #boss [[8. The Eighth Heroine Dungeon]]

```statblock
image: [[Wikilink To Image]]
name: Malicious Spirit Naga
size: Large
type: Monstrosity
alignment: Chaotic Evil
ac: 15
hp: 75
hit_dice: 10d10 + 20
speed: 40 ft.
stats: [18, 17, 14, 16, 15, 16]
saves:
  - Dex: 6
  - Con: 5
  - Wis: 5
  - Cha: 6
damage_immunities: thunder
damage_resistances: poison, acid; bludgeoning, piercing, and slashing damage from non-magical weapons
condition_immunities: charmed, poisoned
senses: darkvision 60 ft., passive Perception 12
languages: --
cr: 8
spells:
  - "The naga is a 10th-level spellcaster. Its spellcasting ability is Intelligence (spell save DC 14, +6 to hit with spell attacks), and it needs only verbal components to cast its spells. It has the following wizard spells prepared:"
  - Cantrips (at will): mage hand, minor illusion, thunderclap
  - 1st level (4 slots): charm person, detect magic, sleep, thunderwave
  - 2nd level (3 slots): detect thoughts, hold person
  - 3rd level (3 slots): lightning bolt, water breathing
  - 4th level (3 slots): blight, thunder step
  - 5th level (2 slots): dominate person
traits:
  - name: Rejuvenation.
    desc: "If it dies, the naga returns to life in 1d6 days and regains all its hit points. Only a wish spell can prevent this trait from functioning."
actions:
  - name: Bite.
    desc: "Melee Weapon Attack: +7 to hit, reach 10 ft., one creature. Hit: 7 (1d6 + 4) piercing damage, and the target must make a DC 13 Constitution saving throw, taking 31 (7d8) poison damage on a failed save, or half as much damage on a successful one."
reactions:
  - name: Unnerving Mask.
    desc: "When a creature the naga can see starts its turn within 30 ft of the naga, the naga can create the illusion that it looks like one of the creature's departed loved ones or bitter enemies. If the creature can see the naga, it must succeed on a DC 14 Wisdom saving throw or be frightened until the end of its turn."
```

### References

* https://www.dndbeyond.com/monsters/17019-spirit-naga
